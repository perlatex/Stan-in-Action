...
Stan 是当前主流的概率编程语言，主要用于贝叶斯推断，它使用先进的 Hamiltonian Monte Carlo (HMC)采样技术(非MCMC)，允许复杂的贝叶斯模型快速收敛，在社会学、生物、医学、物理、工程和商业等领域有广泛的应用。本报告介绍贝叶斯统计推断的数学原理以及Stan应用，并通过案例演示Stan在统计建模中的强大功能。
...
校长很显然不满意，因为随机选择的这200个学生，相对全校学生而言，是一个很小的样本，难免以偏概全了。这个164.89可靠性有多高，或者不确定性是多少？你看的校长脸色有些不好看，弱弱地说：“要不我重新再找200个学生，再做一次，或者再找第三组的200个学生，然后第四组，这样重复很多次”，你怕校长不明白你的意思，还把想法画成思维导图
...
置信区间，是频率学的词汇。关于置信区间，要多说两句，我们想统计的是全校同学的身高均值，这是我们的目标，对应到这张图中，这个黑色圆点，它代表着全校同学的平均身高，它是客观存在的，并且有一个确定的值（只是我们不知道）。
...
```{r, out.width='85%', fig.align='center', echo = F}
knitr::include_graphics("./images/bayesian_vs_frequentist_meme.png")
```
...
看看这个等式，感觉技术上有可操作性了，没错，它就是贝叶斯数据分析的**灵魂**，我们做贝叶斯数据分析都是仰仗这个等式。
...
这两点没有问题，因为符合现实情况，合情合理。现在把我们的猜测画出来就是这样的，
...
第二步，我们需要构建一个**参数空间**，类似网格一样的东西。具体做法是，先指定参数的范围，大一点没关系，然后把这个范围内的所有**可能参数组合**都罗列出来，比如这里构建 1000x1000 个(mu, sigma)参数空间 （类似九九乘法表）
...
(不是说均值是正态分布的么，那么160出现的概率是多少，161出现的概率是多少，当然最后要求各对数)
...
第四步，在参数空间里，计算每个参数组合所对应的分布下，观察到的200个身高值的**对数似然**之和，即下面等式红色部分
$$
...
第三点，这点很重要，因为一般来说，我们不会单独使用Stan，而是在R语言里，完成数据规整变型后，喂给Stan，Stan返回样本后，可以接着在R里统计分析。也就说，**Stan可以当作你已经掌握的数据分析工具的一种插件，当作R语言的一种扩展和增强。** 
...
假定身高和体重满足线性关系，数学表达式如下，我**强烈推荐**这样写
...
这里不是单纯的两个独立的回归分析，而是分成男孩和女孩两组，模型中我们既考虑组内的变化，又要考虑组与组的之间的关联。因此，多层模型写成如下形式
...
这个问题，和李教授给我布置的作业，有点类似，应该是多层和非线性的组合。我们这里的表达式要简单很多。
